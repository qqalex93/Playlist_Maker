...
import com.practicum.playlistmaker.support.debounce
...
class SearchViewModel(
        ...
        onTrackClickDebounce =
            debounce<Int>(ON_TRACK_CLICK_DELAY_MILLIS, viewModelScope, false) { trackId ->
                ...
                if (track != null) {
                    ...
                    screenStateLiveData.value = SearchScreenState.OnTrackClickedEvent(trackId)
                }
            }
    }
    ...
    fun onSearchLineTextChanged(newSearchRequest: String) {
        val currentSearchRequest = newSearchRequest.trim()
        if (currentSearchRequest == latestSearchRequest) return
        latestSearchRequest = currentSearchRequest

        searchJob?.cancel()

        if (currentSearchRequest.isEmpty() && historyTrackList.isNotEmpty() && isDisplayHistoryAllowed) {
        ...
        searchJob = viewModelScope.launch {
            ...
            searchTrack(currentSearchRequest)
        }
    }

    fun searchTrack(newSearchRequest: String? = null) {
        screenStateLiveData.postValue(SearchScreenState.Loading)
        val currentSearchText = (newSearchRequest ?: latestSearchRequest)

        viewModelScope.launch {
        trackSearchInteractor.trackSearch(currentSearchText)
            .collect { pair ->
                processSearchResult(
